 ...
 * 优化实现：请在此处实现你的卷积函数。
 */
void my_calculateKernelResponse(int i, int j, int n, float *src, float *kernel, float *dst, int k_size, int k_rad) {
    dst[i * n + j] = 0.0f;
    // 卷积核中心对称，ki, kj 从 0 到 KERNEL_SIZE-1
    int ki, kj;
    float pixel_value;
    for (kj = 0; kj < k_size; ++kj) { //! 列优先访问，性能较差
        for (ki = 0; ki < k_size; ++ki) {
            ...
            dst[i * n + j] += pixel_value * kernel[ki * k_size + kj];//! 读写内存开销较大
        }
    }
}
...
{    
    int i, j, k_size = KERNEL_SIZE, k_rad = KERNEL_RADIUS;
    for (j = 0; j < n; ++j) {//! 列优先访问，性能较差
        for (i = 0; i < n; ++i) {
            // 对每个输出像素 (i,j)，计算卷积核覆盖区域的加权和
            my_calculateKernelResponse(i, j, n, src, kernel, dst, k_size, k_rad);//! 调用函数（增加函数调用开销）
        }
    }
}
